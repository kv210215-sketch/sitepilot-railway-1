...
const CREDS = {
  email: 'solomiya.demo@sitepilot.dev',
  password: 'Solomiya!Demo2026',
  name: 'Solomiya Energy Demo',
};
...
const out = { api: API, creds: { email: CREDS.email, password: CREDS.password }, steps: {} };
...
async function main() {
  ...
  out.steps.auth = { status: auth.status, userId: auth.json.user?.id, email: auth.json.user?.email };
